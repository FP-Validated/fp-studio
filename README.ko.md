...
**만들어낸** 문자열(인쇄된 값·축 눈금), 잘라낸 copy와 독자가 실제로 보는 문자열. 저자가 쓴
copy는 되돌려주지 않는다 — 이미 문서에 있다. 원본이 `146`인데 `100`이 인쇄됐다면 아무도 쓰지
않은 문자열이므로 `derived`에 나타난다. 리비전과 함께 저장되므로 다음 턴에서 다시 렌더할
필요도 없다.

같은 대화가 `reference` 모양을 알아내려고 무관한 문서의 `/reference`를 3콜에 걸쳐 읽은 것도
같은 종류의 결함이다. 재현 게이트가 필드 단위로 비교하며 불일치를 거부하는데, 그 전사
블록의 모양은 어디에도 공개되지 않았다. `fp_guide('draw', name, redraw=True)`가 이제 그
계약(허용 필드, kind별 블록 모양, 게이트가 비교하는 키)을 함께 돌려준다.
