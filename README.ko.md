...
0.3.9는 **같은 값을 두 번 청구하던 네 곳**을 막았다. 모든 도구 결과는 이후 모든 호출에서 다시
실리므로, N번째 중 k번째 호출의 결과는 `N−k`번 더 청구된다. (1) 최종 검수 체크리스트(표 약
12,500자, 재현 약 21,400자)에는 중복 차단이 없어서 두 번째 호출이 전문을 다시 실었다 — 이제
digest와 항목 수, 포인터만 돌아오고 `again=True`가 본문을 강제한다. (2) 재현 문서의 `review`가
편집 브리프와 캡처된 주장(claim)을 요구했다. 재현은 리서치를 하지 말라고 지시해 놓고 리뷰는
리서치를 요구했고, `/reference` 아래 전사까지 미결 수치로 세어 사용자가 준 숫자를 두 번 청구했다 —
이제 재현은 자신이 전사한 원본에 대해서만 판정되고(414자 → 156자), 리서치를 지어내지 않고도
발행된다. 발행 영수증에는 어떤 원본을 다시 그렸는지 기록된다. (3) 쓰기 결과의 `review`는 포인터
하나당 한 줄씩 40줄까지 붙고 불변 경고를 매번 실었다 — 계열별로 묶어 한 줄로 접고, 경고 전문은
`fp_review`에서만 준다(1,282자 → 408자). (4) 게이트가 "무엇을 검사했는지" 설명하는 고정 문구는
대화당 한 번만 나간다. 24콜 한 턴 기준 재현 277,040자 → 138,267자, 사실 문서 303,096자 →
124,848자.

같은 작업에서 결함 하나가 더 드러났다. 중복 차단 원장이 워크스페이스 DB에 있어서 **대화보다
오래 살았다**: 같은 프로젝트의 두 번째 채팅이 한 번도 받은 적 없는 계약을 두고 "이 대화가 이미
갖고 있으니 스크롤해 올라가라"는 안내를 받았고, 그러면서 렌더 게이트를 통과시키는 digest는
그대로 받았다. 원장은 이제 도구 클로저에 있다 — 세션당 한 번 만들어져 캐시되는 그 수명이 곧
전사(transcript)의 수명이다. 새 대화는 규칙 본문을 다시 받는다.
...
사용자가 원본 이미지 2장을 첨부하고 인포그래픽 2장을 요청한 대화에서, 모델은 웹을 4번
검색하고 페이지 4개를 읽어 **그 페이지의 숫자로** 프레임을 지어냈다. 메시지 안에 원본이
있는데도 그랬다. 그래서 웹 검색·페이지 fetch를 **등록하지 않는다**(지시문으로 말리는 것이
아니라 도구를 없앤다 — 지시문은 바로 옆에 있는 도구와 경쟁해서 진다). 근거는 사용자가
건네준 것뿐이다: 첨부 이미지, 붙여넣은 구조, 허용된 파일, 메시지에 적힌 숫자.

### 원본 2장은 문서 2장이다

왜 모델이 리서치로 갔는지의 진짜 원인은 게이트 자체였다. 게이트는 **렌더 중인 문서가 캡처된
모든 원본을 처리할 것**을 요구했다. 이미지 2장이면 첫 문서는 그 조건을 **절대** 만족할 수
없고, 거부 메시지는 친절하게 해결책을 알려줬다 — `referenceWaiver`를 쓰고 "자유롭게
구성하라". 게다가 waiver는 문자열 하나로 **모든** 원본을 한꺼번에 침묵시켰다. 즉 이 제품의
1원칙을 모델이 스스로 쓴 한 문장으로 해제할 수 있었다.

0.3.6부터: 원본 하나를 재현하는 문서는 통과하고, **아무것도 재현하지 않은 문서**는 대기 중인
원본이 있는 동안 거부된다. 다른 문서가 이미 재현한 원본은 끝난 것으로 본다. waiver는
원본별이고 사용자의 말을 인용해야 한다: `referenceWaiver={"<source_id>": "<사용자가 한
말>"}`. 그리고 **발행**은 재현되지 않은 원본이 하나라도 남아 있으면 거부한다 — 그리는 동안은
한 장씩 진행하고, 납품 시점에 약속을 전부 확인한다.

### 라이트냐 다크냐는 사용자가 정한다

프레임의 모든 색은 테마 팩 하나에서 나온다. 그래서 팩은 추론할 취향이 아니다 — 사용자가
말하기 전에 그린 그림은 결과처럼 보이는 추측이다. 0.3.7부터 `fp_render`는 **선언이 없으면
거부**하고, 거부 메시지는 선언을 만드는 단 하나의 호출(`ask_user`)을 지목한다. 모델이 쓸 수
있는 것으로는 만족시킬 수 없다. 문서 필드는 모델 자신의 글이고, **모델이 짜낸 문장으로 빠져
나갈 수 있는 게이트는 게이트가 아니다**(0.3.6에서 배운 그것이다).

사용자의 말이 들어오는 경로는 둘이고 둘 다 같은 방식으로 기록된다: 타이핑한 메시지는
`capture_turn`으로, `ask_user`의 답은 서버 `question_asker`로. 색 단어 하나는 선언이 아니다 —
그렇지 않으면 "Batch Prover And **Light** Client Prover"가 조용히 종이를 골라버린다. 모드
단어가 붙거나("라이트 모드"), 한국어 조사가 붙거나("다크로"), 질문에 대한 답 길이면 선언이다.

라이트 팩은 다크 팩의 **측정된 차이**다(`extends` + RFC 7386 병합, 4 KB). 560개 필드를 두 벌
복사하면 반드시 드리프트하고, 그 드리프트는 렌더될 때까지 보이지 않는다. 중립 램프는 **순위로
미러링**한다 — 모든 단계가 측정된 브랜드 회색을 유지하고 양 끝이 자리를 바꾸므로, 역할
참조(`neutral.100` = 제목)는 **하나도 바뀌지 않는다**. 유채색은 색상(hue)을 지키고 종이 대비
**3:1**을 넘길 만큼만 명도를 낮춘다. 측정값은 두 규칙을 모두 이기고, 각 항목은 읽어온 Figma
노드 id를 provenance에 남긴다.

라이트판과 다크판은 한 문서의 두 리비전이 아니라 **두 산출물**이다. 기존 문서를 다른 팩으로
렌더하는 것은 거부되고, 거부 메시지가 다른 이름을 쓰라고 말한다.
...
3. 근거는 **사용자가 건네준 것**이다: 첨부 이미지, 붙여넣은 구조, 허용된 파일, 메시지에 적힌 숫자. 이 제품에는 **웹 검색도 페이지 fetch도 없다** — 자료에 없는 수치는 지어내는 것이 아니라 물어보는 것이다.
...
이 게이트가 증명하는 것은 **규칙이 다시 제시되고 답변됐다**는 사실이지, 답변이 참이라는 것이 아니다. 검수 체크리스트는 최종 발행 직전에만 주입되고(표 기준 약 12,500자, 재현 약 21,400자) 렌더·편집 결과에는 절대 붙지 않는다. 0.3.9부터는 **한 대화에 한 번**이다 — 두 번째 `fp_review`는 digest와 항목 수, 포인터만 돌려주고 `again=True`가 본문을 강제한다.
...
0.3.6에서 web_fetch 자체가 사라졌으므로 위 17%는 **기록**이다. 지금 그 비용은 0이고, 그
경로로 들어오던 페이지 본문·excerpt·provenance receipt도 없다. `fp_source_text`는 사용자가
건네준 캡처(첨부·붙여넣기·허용된 파일)를 읽는 데 그대로 쓴다.
...
GUI는 새 세션의 소켓을 **폴더를 알기 전에** 연다. 그래서 `?workspace=`가 빈 값으로 오는
연결이 하필 첫 메시지 — 첨부 이미지와 "화이트 다크 2가지 버전"이라는 선언이 실린 그 턴 —
을 나른다. 서버는 그 빈 값을 보고 캡처를 건너뛰었고, 결과는 기록에 그대로 남아 있다.

- 첨부한 표 이미지가 캡처 원본이 되지 못해, 모델은 이미 받은 그림을 찾아 `fp_inspect` 12회,
  `fp_source` 12회, `grep`, `list_files`를 돌렸다.
- 선언이 기록되지 않아 렌더가 거부되고, 모델이 물었고, 사용자가 "Dark"를 답해 dark만 남았다.
  이어서 "Both"를 두 번 답했지만 그 단어에는 색 이름이 없어 **아무것도 기록되지 않았다**.
  라이트 버전은 세 번 더 거부됐고 사용자가 네 번째 질문을 중단시켰다. 라이트는 끝내 없었다.
- 모델이 렌더 결과를 확인하려 `read_file`로 PNG를 열어 918,976자의 U+FFFD를 받았다. 그 한
  번이 이후 18회 호출마다 재청구돼 그 턴 도구 트래픽 19.0M자 중 16.5M자를 차지했다.

캡처와 `ask_user` 답변은 이제 소켓 파라미터가 아니라 엔진이 쓰는 것과 같은
`manager.engine_workspace(...)`로 워크스페이스를 정한다. "Both"·"둘 다"·"두 버전 모두"는
**질문이 라이트와 다크를 함께 물었을 때** both로 기록된다(문법을 묻는 질문에 "둘 다"는
선언이 아니다). `read_file`은 첫 바이트가 UTF-8 텍스트가 아니면 경로와 크기를 말하고
거부한다. 기록된 44회 턴을 같은 호출 순서로 재계산하면 **18,999,604자 → 2,230,162자(-89%)**다.
