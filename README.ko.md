...
0.3.4는 남은 가장 큰 누출을 막았다: **렌더 결과를 확인할 방법이 SVG를 읽는 것밖에 없었다.**
납품되는 SVG는 글리프가 모두 path로 아웃라인된 것이다 — `<text>`도 없고 `<rect>`도 path가
되어 있어서 읽어도 아무것도 안 나온다. 그런데 실제 대화는 그 SVG를 **두 번** 읽고, 이어서
코드베이스에서 hex 색 두 개를 그렙했다. 어느 계열이 어느 색으로 그려졌는지 알아내려고 한
것이다. 두 번의 읽기는 답을 주지 않은 채 그 턴의 이후 모든 호출에서 다시 청구됐다 — 트레이스
전체의 guide 호출을 합친 것보다 비쌌다.

이제 렌더가 `layout`을 돌려준다: 실제로 확정된 문법과 style, 계열별 색(hex), 렌더러가
**만들어낸** 문자열(포맷된 값·축 눈금), 잘라낸 copy와 독자가 실제로 보는 문자열. 저자가 쓴
copy는 되돌려주지 않는다 — 이미 문서에 있다. 리비전과 함께 저장되므로 다음 턴에서 다시 렌더할
필요도 없다. **주의**: 저자가 `100`이라고 써넣은 값은 `derived`에 나타나지 않는다. 그건
렌더러가 만든 문자열이 아니고, 아래의 재현 게이트가 맡는 일이다.

같은 대화가 `reference` 모양을 알아내려고 무관한 문서의 `/reference`를 3콜에 걸쳐 읽은 것도
같은 종류의 결함이다. 전사 블록의 모양이 어디에도 공개되지 않았다. `fp_guide('draw', name,
redraw=True)`가 이제 그 계약(허용 필드, kind별 블록 모양, 게이트가 비교하는 키)을 함께
돌려준다.

### 행에 있는 모든 숫자가 데이터다

0.3.4까지 재현 게이트의 숫자 검사는 **키 이름 목록**(`value`, `values`, `percent`, …)만
봤다. 그런데 문법은 열 이름을 저자가 정한다: `fields: {value: 'Index'}`라고 쓰면 프레임의
모든 숫자가 `Index` 아래로 들어가 **게이트에 보이지 않는다.** 그래서 원본이 146·22·48·839.8M
이라고 말하는데 한 계열을 100으로 지수화해 같은 길이 막대 7개를 그린 프레임이 그대로 커밋됐다.
게이트는 아무 말도 하지 않았다.

0.3.5부터 **행 안의 모든 숫자가 데이터다** — 저자가 그 열을 무엇이라 불렀든. 문자열 안의
숫자도 읽고 접미사 크기(`839.8M` → 839,800,000)까지 같은 값으로 취급하므로, 원본 문자열을
그대로 둔 재현과 숫자로 펼친 재현이 모두 통과하고 지수화는 거부된다. 그리고 `fields`는
역할→열 이름 매핑이므로 더 이상 copy로 읽지 않는다(`'measure'`가 "이름을 바꿨다"로 잡히던
거짓 경고). 범례로 그려지는 `fields.columns`만 copy로 검사한다.

이미 커밋된 리비전도 확인한다: `fp_inspect`가 저장된 문서의 `reference_drift`를 함께
돌려준다. 그 프레임이 원본과 어긋나 있다는 사실을 사람이 원본을 옆에 놓고 비교해야만 알 수
있던 상태를 끝낸다.
...
OpenWorker 원본은 `5bc10d928e0b64aae74313349a3b17bd19643ae2`, FP kit는 `2a22c2632177c5b8bea397169ec7fcbfcd664688`에 고정한다. 원본 anchor가 다르면 조립을 중단한다. npm의 전이 의존성 lock은 해당 설치에서 생성/검증하고 release에 보관해야 한다. 이 archive는 확인하지 못한 lock/integrity 값을 만들어 넣지 않았다.
