...
Local validation: 2,182 regression tests pass in an assembled tree (every FP suite plus the upstream suite, real resvg compiler and supplied fonts) with 1 skipped, and 53 fp-kit tests pass. Live model calls remain unexecuted here; the signature, notarization and hash of a published DMG are recorded in that release's notes, not claimed by this archive.
